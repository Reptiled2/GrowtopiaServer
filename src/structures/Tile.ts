  ...
  public serialize(actionType: number) {
    ...
    switch (actionType) {
      ...
      case ActionTypes.LOCK: {
        ...
        const admins = (this.block.worldLock ? this.world.data.admins : this.block.lock?.adminIDs) || [];

        let count = 0;
        
        buf = Buffer.alloc(26 + 4 * admins.length);
        ...
        buf.writeUInt32LE(admins.length, 14); // admin count
        admins.forEach(adminID => {
          buf.writeUInt32LE(adminID, 18 + 4 * count);
          count++;
        })
        
        buf.writeInt32LE(-1, 18 + 4 * admins.length);
