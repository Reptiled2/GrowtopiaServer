import { Variant, TankPacket, TextPacket, ItemsDat } from "growtopia.js";
...
export default class extends Listener<"raw"> {
  ...
  private sendGuest(peer: Peer, requestedName: string) {
    const dialog = new DialogBuilder()
      ...
      .endDialog("register_end", "", "Create").str();
          ...
          this.base.database.getUser(username).then((user) => {
            if (!user || password !== decrypt(user?.password)) {
              ...
              peer.send(TextPacket.from(DataTypes.ACTION, "action|set_url", "url||https://127.0.0.1/recover", "label|`$Recover your Password``"), 
                TextPacket.from(DataTypes.ACTION, "action|logon_fail"));
            ...
            peer.data.state = peer.checkState();

            peer.data.ownedWorlds = user.ownedWorlds?.length ? JSON.parse(user.ownedWorlds.toString()) : [];
            peer.data.favorites = user.favorites ? JSON.parse(user.favorites.toString()) : {
              savedWorlds: [],
              home: "START"
            };

            peer.data.cooldowns = user.cooldowns ? JSON.parse(user.cooldowns.toString()) : {
              sb: 0
            };
        ...
        switch (tank.data?.type) {
          ...
          case TankTypes.PEER_CLOTH: {
            ...
            if (item?.type === ActionTypes.LOCK) {
              const inventoryItemWL = peer.data.inventory.items.find(v => v.id === 242);
              const inventoryItemDL = peer.data.inventory.items.find(v => v.id === 1796);

              switch (item.id) {
                case 242:
                  if (inventoryItemWL?.amount && inventoryItemWL.amount >= 100) {
                    if (inventoryItemDL?.amount && inventoryItemDL.amount === 200) {
                      peer.send(Variant.from("OnTalkBubble", peer.data.netID, "You can't transform your `$World Locks ``into `$Diamond Lock` `! Inventory full."));
                      return;
                    }
                ...
                case 1796:
                  if (inventoryItemWL?.amount && inventoryItemWL.amount > 100) {
                    peer.send(Variant.from("OnTalkBubble", peer.data.netID, "You can't transform your `!Diamond Lock ``into `$World Locks``! Inventory full."));
                    return;
                  }
                ...
                case 7188:
                  if (inventoryItemDL?.amount && inventoryItemDL.amount > 100) {
                    peer.send(Variant.from("OnTalkBubble", peer.data.netID, "You can't transform your `3Blue Gem Lock ``into `!Diamond Locks``! Inventory full."));
                    return;
                  }
            ...
            /*if (item && [1796, 242, 202, 204, 206, 4994, 9640, 7188, 2408, 2950, 4428, 4802, 5814,
              5260, 5980, 8470, 10410, 11550, 11586, 11902, 12654, 13200
            ].includes(item.id)) {
              break;
            }*/
            switch (item?.bodyPartType) {
              ...
              case ClothTypes.BACK: {
                ...
                if (peer.data.clothing.back === tank.data.info) {
                  ...
                  peer.data.clothing.back = tank.data.info || 0;
                }
            ...
            peer.sound("audio/change_clothes.wav");
              ...
              peer.everyPeer((p) => {
                if (p.data?.netID !== peer.data?.netID && p.data?.world === peer.data?.world && p.data?.world !== "EXIT") {
                  p.send(
                    ...
                    Variant.from("OnConsoleMessage", `\`5<${peer.getTag(world)}\`\` left, \`w${world?.data.playerCount}\`\` others here\`5>\`\``),
                    Variant.from("OnTalkBubble", peer.data.netID, `\`5<${peer.getTag(world)}\`\` left, \`w${world?.data.playerCount}\`\` others here\`5>\`\``),
